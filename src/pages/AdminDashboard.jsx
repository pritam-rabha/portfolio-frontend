...
function StatCard({ label, value, sub, accent }) {
  return (
    <div className={`card p-5 ${accent ? 'border-volt-500/25 bg-gradient-to-br from-volt-500/5 to-transparent' : ''}`}>
      ...
      <p className={`font-display font-semibold text-3xl mt-2 ${accent ? 'text-volt-500' : 'text-white'}`}>{value}</p>
      ...
      <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }}
        ...
        <form onSubmit={submit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <textarea value={form.description} onChange={upd('description')} required rows={3} placeholder="What does it do?" className="field text-sm resize-none" />
          </div>
          ...
          <div className="flex items-center gap-5">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.featured} onChange={upd('featured')} className="w-4 h-4 rounded border-white/20 bg-ink-700 accent-volt-500" />
              <span className="text-sm text-ash-300">Featured</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.isPublished} onChange={upd('isPublished')} className="w-4 h-4 rounded border-white/20 bg-ink-700 accent-volt-500" />
...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-ink-950 flex">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="card p-5">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={() => setTab('messages')} className="text-xs text-ash-500 hover:text-volt-400 transition-colors flex items-center gap-1">
                ...
                <div className="card p-5">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={() => setTab('projects')} className="text-xs text-ash-500 hover:text-volt-400 transition-colors flex items-center gap-1">
                      View all <ChevronRight size={12} />
                    </button>
                  </div>
                  <div className="space-y-2">
          ...
          {tab === 'projects' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
              <div className="flex justify-end mb-5 md:hidden">
                <button onClick={() => setShowForm(true)} className="btn-volt text-xs px-3 py-2"><Plus size={13} /> New</button>
              </div>
              {loadingP ? (
                ...
                <div className="text-center py-20">
                  ...
                  <button onClick={() => setShowForm(true)} className="btn-volt text-sm"><Plus size={14} /> Create your first project</button>
                </div>
              ) : (
                <div className="space-y-3">
                  {projects.map(p => (
                    <div key={p._id} className="card p-4 flex items-center gap-4 hover:border-white/10 transition-all group">
                      <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 flex-shrink-0">
                        ...
                        <button onClick={() => handleDeleteProject(p._id)} className="p-1.5 text-ash-500 hover:text-ember-400 transition-colors rounded-lg hover:bg-ink-600"><Trash2 size={14} /></button>
                      </div>
                    </div>
                  ))}
                </div>
                            ...
                            <div className="px-6 pb-5 border-t border-white/[0.06] pt-4">
                              ...
                              <div className="border border-white/[0.08] rounded-xl p-4 bg-ink-800/50">
                                ...
                                <textarea
                                  ...
                                  onChange={e => setReplyText(r => ({ ...r, [m._id]: e.target.value }))}
                                  rows={4}
                                  placeholder="Write your reply here…"
                                  className="field resize-none text-sm w-full mb-3"
                                />
                                <div className="flex items-center gap-2">
                                  <button
                                    onClick={() => handleSendReply(m._id)}
                                    disabled={sendingReply[m._id]}
                                    className="btn-volt text-xs px-4 py-2 disabled:opacity-60"
                                  >
                                    {sendingReply[m._id]
                                  ...
                                  <button onClick={() => handleDeleteMessage(m._id)}
                                    className="ml-auto p-1.5 text-ash-500 hover:text-ember-400 transition-colors rounded-lg hover:bg-ink-600">
                                    <Trash2 size={13} />
                                  </button>
                                </div>
                              </div>
      ...
      <AnimatePresence>
        {showForm && <ProjectForm onClose={() => setShowForm(false)} onCreated={fetchProjects} toast={toast} />}
      </AnimatePresence>
    </div>
  )
}